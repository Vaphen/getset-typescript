...
module.exports = {
  ...
  generateGetterFor(varName, type,  intendationchar = ' ', intendationdepth = 1) {
    if(!type || type.length === 0) {
      type = 'any';
    }
    return(
...
\t`
  ...
  generateSetterFor(varName, type) {
    if(!type || type.length === 0) {
      type = 'any';
    }
    return(
`\tset ${varName}(new${varName[0].toUpperCase() + varName.slice(1)}: ${type}) {
