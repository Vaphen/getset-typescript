...
module.exports = {
  extractVarName(str, privatePrefix) {
    return getPartBetween(str.trimAll(), privatePrefix, [':', ';', '\n', '=']);
  },
...
\t}`
  ...
  generateSetterFor(varName, type, privatePrefix) {
    ...
    return(
`\tset ${varName}(${varName[0].toUpperCase() + varName.slice(1)}: ${type}) {
\t\tthis.${privatePrefix}${varName} = ${varName[0].toUpperCase() + varName.slice(1)};
\t}`
    )
  },
}
